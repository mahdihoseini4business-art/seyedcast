...
class Seyedcast_Pwa {
	...
	private function output_sw() {
		...
		$settings    = Seyedcast_Settings::get();
		$sw_title    = wp_json_encode( ! empty( $settings['pwa_name'] ) ? $settings['pwa_name'] : 'Seyedcast' );
		...
		echo 'const DEFAULT_TITLE = ' . $sw_title . ";\n";
		...
		echo "  var data = { title: DEFAULT_TITLE, body: '', url: '/', icon: '', tag: 'seyedcast' };\n";
		echo "  try {\n";
		echo "    if (event.data) {\n";
		echo "      var parsed = event.data.json();\n";
		echo "      if (parsed && typeof parsed === 'object') {\n";
		echo "        data.title = parsed.title || data.title;\n";
		echo "        data.body = parsed.body || '';\n";
		echo "        data.url = parsed.url || '/';\n";
		echo "        data.icon = parsed.icon || '';\n";
		echo "        data.tag = parsed.tag || data.tag;\n";
		echo "      }\n";
		echo "    }\n";
		echo "  } catch (e) {\n";
		echo "    try { data.body = event.data ? event.data.text() : ''; } catch (e2) {}\n";
		echo "  }\n";
		echo "  event.waitUntil(self.registration.showNotification(data.title, {\n";
		echo "    body: data.body,\n";
		echo "    icon: data.icon || undefined,\n";
		echo "    badge: data.icon || undefined,\n";
		echo "    tag: data.tag,\n";
